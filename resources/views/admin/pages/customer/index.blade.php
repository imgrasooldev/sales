    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-11 pad-zero">
                ...
                <div class="mainWrap">
                    <a href="{{ Route('admin.create_customers') }}" style="margin: 10px" class="btn btn-success">New
                        Customer</a>
                    <div style="margin: 10px" class="">
                        <div class="allpaid">
                            <div style="display: flex; justify-content: space-between">
                                <h4>Customers</h4>
                                <form action="{{ Route('admin.filter') }}" method="get" style="display: flex;">
                                    @csrf
                                    <select name="brandFilter" id="brand" class="form-control" style="margin-right: 10px">
                                        <option value="">*** Select Brand ****</option>
                                        @foreach ($brands as $brand)
                                        @if(isset($data['brandFilter']))
                                        <option {{ $brand->id == $data['brandFilter']?'selected':''}} value="{{ $brand->id }}">{{ $brand->name }}</option>
                                        @else
                                    ...
                                    <div style="display: flex">
                                        <label style="font-size: 15px; margin-right: 5px; color: gainsboro">From </label>
                                        <input class="form-control border" value="{{ isset($data['from'])?$data['from']:'' }}" type="date" name="from" id="from">
                                        <label style="font-size: 15px; margin-right: 5px; color: gainsboro; margin-left: 5px">To </label>
                                        <input class="form-control border" type="date" value="{{ isset($data['to'])?$data['to']:'' }}" name="to" id="to">
                                        <div style="margin-right: 5px; margin-left: 10px">
                                            <button style="border: #ffffff; background-color: #f16f23; color: white; padding: 8px 20px">Apply</button>
                                        </div>
                                        <div>
                                            <a href="{{ Route('admin.get_customers') }}" style="border: #ffffff; background-color: #f16f23; color: white; padding: 8px 20px">cancel</a>
                                        </div>
                                    </div>
